      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div className="max-w-xl">
            <span className="px-4 py-1.5 bg-slate-200 text-slate-600 text-xs font-bold rounded-full uppercase tracking-widest">Our Advantages</span>
            <h2 className="text-4xl md:text-5xl font-bold text-slate-900 mt-6 leading-tight tracking-tighter">
              <TextType
                ...
                showCursor
                cursorCharacter="|"
                text={["Cukup Datang", "Masukkan", "[[Terima Uangnya.]]"]}
                deletingSpeed={50}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {sellerAdvantages.map((item, index) => (
            <div key={index} className="bg-[#F1F3F5] rounded-[40px] p-10 flex flex-col justify-between h-125 hover:bg-white hover:shadow-2xl hover:shadow-slate-200 transition-all duration-300 group">
              <div className="mb-10 flex justify-center items-center group-hover:scale-105 transition-transform duration-500">{item.mockup}</div>

              <div>
                <div className={`w-12 h-12 ${item.color} rounded-2xl flex items-center justify-center mb-6 shadow-sm`}>{item.icon}</div>
